...
folder_to_monitor = "/Users/hp/Desktop"
folder_to_move_latefiles = folder_to_monitor + "/" + dumping_folder
threshold_days = 5
code_location = "C:/Users/hp/Desktop/abayomi_personal/Desktop_Cleaner"
...
class TheHandler(FileSystemEventHandler):
    def on_modified(self, event):
        ...
        for each_file in os.listdir(folder_to_monitor):
            ...
            if (
                os.path.isfile(each_file_path)
                and os.path.splitext(each_file_path)[-1].lower() != ".lnk"
            ):
        ...
        if each_file != dumping_folder:
            ...
            if os.path.isfile(incoming_file_path):
                ...
                while incoming_file_path:
                    i += 1
                    new_name = (
                        os.path.splitext(folder_to_monitor + "/" + new_name)[0]
                        + str(i)
                        + os.path.splitext(incoming_file_path)[-1]
                    )
                    new_name = new_name.split("/")[4]
                    incoming_file_path = os.path.isfile(
                        folder_to_move_latefiles + "/" + new_name
                    )
            ...
            print(
                each_file
                + " was successfully moved to gabbage after spending "
                + str(expended_days)
                + " days on your desktop!"
            )
...
if connect():
    toast.show_toast(
        "eBayo's Automated Machine",
        "There is internet and your Desktop is Clean with me, Abayomi",
        duration=30,
    )
else:
    toast.show_toast(
        "eBayo's Automated Machine",
        "No Internet but your Desktop is Clean with me, Abayomi",
        duration=30,
    )
